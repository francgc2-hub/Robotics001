...
robot.plot(q=[np.deg2rad(joint1), np.deg2rad(joint2)], limits=[-0.8,0.8,-0.8,0.8,-
0.1,0.2], backend='pyplot', shadow=True, jointaxes=True, block=True)
...
#Cinemática inversa
we=[1, 1, 1, 0, 0, 0]
sol = robot.ikine_LM(Tep=q, q0=[0,0], ilimit=5000, slimit=1000, tol=0.000005,
joint_limits=True, mask=we)
print(sol)
# #Mostrar resultado
if sol.success:
    ...
    robot.plot(q=sol.q, limits=[-0.8,0.8,-0.8,0.8,-0.1,0.2], backend='pyplot',
    shadow=True, jointaxes=True, block=True)
